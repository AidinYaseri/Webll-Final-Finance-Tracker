...
router.get('/', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    const result = await pool.query(
      `SELECT t.transaction_id, t.user_id, t.amount, t.type, t.category_id, c.name as category_name, t.date, t.description, t.created_at
       FROM transactions t
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO transactions (user_id, amount, type, category_id, date, description)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING transaction_id, user_id, amount, type, category_id, date, description, created_at`,
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE transactions 
       ...
       RETURNING transaction_id, user_id, amount, type, category_id, date, description, updated_at`,
